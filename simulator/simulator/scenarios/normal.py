...
def update_state(
	...
	elapsed_sec = phase_tick / max(1, hz)
	...
	in_warning = _is_warning_segment(progress)
	allowed_speed = 55.0 if in_warning else 80.0
	...
	state["gradient_permille"] = 4.0 if in_warning else 0.0
	state["active_fault_codes"] = ["WARNING_SEGMENT"] if in_warning else []
